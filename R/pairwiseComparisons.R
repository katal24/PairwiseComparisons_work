...
uniqueTriadsTuplesAndIdx <- function(matrix){
  ...
  t(rbind(tuples, triads, triadIdxs))
}
...
#' @return the closest consistenr triad
#' @export
...
#' @param ... - matrices od vectors the same dimension
#' @return aggregation matrix/vector
#' @export
AIJadd <- function(...){
  matrixes <- list(...)
  Reduce('+', matrixes)/length(matrixes)
  matrixes
}
...
AIJgeom <- function(...){
  matrixes <- list(...)
  Reduce('*', matrixes)^(1/length(matrixes))    # Reduce - wywoluje funkcje po kolei na elementach listy
  matrixes
}
...
#' @return the list of indices that violate the first Condition of Order Preservation
#' @export
  ...
  for(i in 1:dim(matrix)[1]){
    for(j in 1:dim(matrix)[2]){
      ...
      if(i!=j && (element > 1 && resultList[i]<resultList[j]) || (element < 1 && resultList[i]>resultList[j])){
        errorList[[length(errorList)+1]] <- c(i,j)
      }
    }
  }
  errorList
...
#' @param matrix - PC matrix
#' @param resultList - ranking of matrix
#' @return the list of indices that violate the second Condition of Order Preservation
#' @export
